    ...
    def run(self) -> None:
        ...
        try:
            ...
            if not hasattr(self._device, "get_status"):
                self.signals.poll_failed.emit(self._device_id, "invalid_device", "设备对象缺少get_status方法")
                return
            ...
            if data and isinstance(data, dict):
                ...
                try:
                    ...
                    _persist_logger.exception("Data persistence failed for device %s", self._device_id)

                # 发送成功信号（跨线程安全）
                self.signals.poll_success.emit(self._device_id, data, elapsed_ms)

                # 4️⃣ 性能监控：如果响应时间过长，发出警告
                if elapsed_ms > 100:  # >100ms 认为异常慢
                    self.signals.performance_warning.emit(
                        self._device_id, f"轮询响应时间过长: {elapsed_ms:.1f}ms (正常<50ms)"
                    )

            else:
                # ❌ 无数据返回（超时或协议错误）
                self.signals.poll_timeout.emit(self._device_id, elapsed_ms)

        except ConnectionError as conn_err:
            # 网络连接断开
            elapsed_ms = (time.monotonic() - start_time) * 1000
            self.signals.poll_failed.emit(self._device_id, "connection_error", f"连接错误: {str(conn_err)}")
            ...
            self.signals.poll_failed.emit(self._device_id, error_type.lower(), error_msg)
