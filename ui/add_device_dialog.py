...
class ConnectionPage(QWizardPage):
    ...
    def _update_dynamic_params(self, protocol: ProtocolType) -> None:
        ...
        while self.param_layout.rowCount() > 0:
            item = self.param_layout.itemAt(0, QFormLayout.ItemRole.LabelRole)
            if item and item.widget():
                item.widget().deleteLater()
            item = self.param_layout.itemAt(0, QFormLayout.ItemRole.FieldRole)
            if item and item.widget():
                item.widget().deleteLater()
    ...
    def _on_test_connection(self) -> None:
        ...
        QMessageBox.information(self, "Test Result", "Connection test completed (simulated)")
...
class RegisterMapPage(QWizardPage):
    ...
    def _init_ui(self) -> None:
        ...
        self.table.setHorizontalHeaderLabels(["Name", "Data Type", "Address", "Writable", "Decimals", "Scale", "Unit"])
    ...
    def validatePage(self) -> bool:
        ...
        if self.table.rowCount() == 0:
            QMessageBox.warning(self, "Incomplete Configuration", "Please configure at least one data point!")
            return False
        return True
