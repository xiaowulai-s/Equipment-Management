...
class MainWindowV2(QMainWindow):
    ...
    def _reposition_edge_buttons(self) -> None:
        ...
        # 右侧面板 (报文生成工具) 按钮：位于整个窗口右边边沿或面板左边缘, 垂直居中且与左侧按钮高度统一
        if hasattr(self, "_modbus_generator") and hasattr(self, "_modbus_expand_btn") and hasattr(self, "_modbus_collapse_btn"):
            # 获取整个窗口的几何信息
            window_geo = self.geometry()
            # 获取central_widget在窗口中的位置
            central_pos = self._central_widget.pos()
            
            # 计算垂直居中位置，与左侧面板按钮高度统一
            # 使用与左侧按钮相同的Y坐标，确保高度一致
            modbus_y = self._collapse_btn.y()
            ...
            if modbus_geo.width() > 0:
                ...
                # 如果面板宽度为0（隐藏状态），按钮位于整个窗口右边边沿
                modbus_x = window_geo.width() - 10  # 按钮宽度一半覆盖边缘，位于整个窗口右侧
            
            # 调整X坐标，考虑central_widget在窗口中的位置
            modbus_x = modbus_x - central_pos.x()

            # 确保按钮在central_widget范围内
            central_width = self._central_widget.width()
            central_height = self._central_widget.height()
            modbus_x = max(0, min(modbus_x, central_width - 10))
            modbus_y = max(0, min(modbus_y, central_height - 24))
