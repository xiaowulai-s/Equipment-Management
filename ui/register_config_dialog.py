...
class RegisterConfigDialog(QDialog):
    ...
    def _delete_register(self):
        ...
        reply = QMessageBox.question(
            self, "确认删除", f"确定要删除地址 {address} 的寄存器吗？", QMessageBox.Yes | QMessageBox.No
        )
    ...
    def _move_up(self):
        ...
        if row > 0:
            # 交换寄存器
            self._register_map[row], self._register_map[row - 1] = self._register_map[row - 1], self._register_map[row]
            self._load_registers()
            # 选中上移后的行
